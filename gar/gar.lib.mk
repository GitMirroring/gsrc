...
checksig-%.sign:
	@printf "[$(OK)checksig$(OFF)] $(MSG)Checking GPG signature $(OFF)$*\n"
	$(GPGV) $(DOWNLOADDIR)/$*.sign   $(DOWNLOADDIR)/$*
	$(MAKECOOKIE)

checksig-%.asc:
	@printf "[$(OK)checksig$(OFF)] $(MSG)Checking GPG signature $(OFF)$*\n"
	$(GPGV) $(DOWNLOADDIR)/$*.asc  $(DOWNLOADDIR)/$*
	$(MAKECOOKIE)

checksig-%.md5sum:
	@printf "[$(OK)checksig$(OFF)] $(MSG)Checking MD5sum checksums $(OFF)$*\n"
...
# as currently written, doesn;t deal with git downloads
garchive-%:
	@printf "[$(OK)garchive$(OFF)] $(MSG)Copy to cache $(GARCHIVEDIR) $(OFF)\n"
...
PERL_CONFIGURE_ARGS ?= INSTALL_BASE=$(prefix)

configure-%/Makefile.PL:
	@printf "[$(OK)configure$(OFF)] $(MSG)Configuring Makefile.PL in $(OFF)$*\n"
	cd $* && $(CONFIGURE_ENV) perl Makefile.PL $(PERL_CONFIGURE_ARGS) $(OUTPUT)
	$(MAKECOOKIE)
...
build-%/Makefile build-%/makefile build-%/GNUmakefile:
	...
	@$(BUILD_ENV) $(MAKE) -C $* $(MAKE_ARGS) $(BUILD_ARGS) $(OUTPUT) ||  $(BUILD_FAIL)
	$(MAKECOOKIE)
...
# FIXME fix other place NORMALIZE
...
install-%/Makefile install-%/makefile install-%/GNUmakefile:
	@printf "[$(OK)install in $* $(OFF)] $(MSG)Running make $(INSTALL_TARGET) from $(OFF)$*\n"
	$(INSTALL_ENV) $(MAKE) -C $* $(MAKE_ARGS) $(INSTALL_ARGS) $(INSTALL_TARGET) $(OUTPUT)
	$(MAKECOOKIE)

# Use "make install" with DESTDIR.   ha~=!  packageDESTDIR or packageDISTDIR/$(prefix) is realDESTDIR
DESTDIR_INSTALL_ARGS ?=
install-%/Makefile-DESTDIR:
	@printf "[$(OK)install$(OFF)] $(MSG)Running make $(INSTALL_TARGET) to DESTDIR from $(OFF)$*\n"
	$(INSTALL_ENV) $(MAKE) -C $* $(MAKE_ARGS) $(DESTDIR_INSTALL_ARGS) $(INSTALL_TARGET) DESTDIR=$(packageDESTDIR) $(OUTPUT)
	$(MAKECOOKIE)

install-%/Makefile-BSD:
	@printf "[$(OK)install in $* w pmake$(OFF)] $(MSG)Running pmake $(INSTALL_TARGET) from $(OFF)$*\n"
...
install-%/setup.py:
	@printf "[$(OK)install$(OFF)] $(MSG)Running python setup.py install in $(OFF)$*\n"
...
install-%/setup.rb:
	@printf "[$(OK)install$(OFF)] $(MSG)Running ruby setup.rb install in $(OFF)$*\n"
...
sysinstall-check:
	...
	@if [ "`ls $(packagedir)`" ]; then \
        printf "[$(OK)sysinstall-check$(OFF)] $(MSG)Successful. packagedir $(packagedir) ready$(OFF)\n"; \
    else \
        printf "[$(ERR)sysinstall-check$(OFF)] $(MSG)packagedir $(packagedir) install failed,  so uninstalling$(OFF)\n"; \
        $(MAKE) -C $(WORKOBJ) uninstall $(OUTPUT); \
        exit 1; \
    fi
	$(MAKECOOKIE)
# why uninstall? is uninstalling from packageDESTDIR since it ups WORKBOJ makefile?

sysinstall-collisions:
	@printf "[$(OK)sysinstall-collisions$(OFF)] $(MSG)Removing collisions$(OFF)\n"
...
sysinstall-normalise:
	...
	@set -e; \
         ...
         if [ -d $$1 ]; then \
          ...
          normalise $(packagedir)/usr  $(packagedir); \
          normalise $(packagedir)/local  $(packagedir); \
...
sysinstall-install:
	...
	(cp -r -v -f --symbolic-link --target-directory=$(prefix) $(packagedir)/* $(OUTPUT) && \
            ln -n -v -f -s $(PACKAGENAME) $(dotgardir)/VERSION $(OUTPUT)) || $(SYSINSTALL_FAIL) $(OUTPUT)

sysinstall-uninstall:
	@printf "[$(OK)sysinstall-uninstall$(OFF)] $(MSG)Unstowing from working directories $(OFF)\n"
	rm -f $(COOKIEDIR)/sysinstall-* $(COOKIEDIR)/*-install $(OUTPUT)
	rm -rf $(COOKIEDIR)/install-* $(OUTPUT)
	(test -d $(dotgardir) || (printf "[$(ERR)sysinstall-uninstall$(OFF)] $(MSG)$(GARNAME) is not installed$(OFF)\n" && exit 1)) && \
	( cd $(prefix) ; \
...
.PRECIOUS: $(DOWNLOADDIR)/% $(COOKIEDIR)/% $(FILEDIR)/% $(DOWNLOADDIRGIT)/%
