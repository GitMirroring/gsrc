...
GARPKGDIR ?= ../..
PKGGROUPS ?= bio freedesktop gnome gnu gnustep  gstreamer hurd  other xorg     external libs tools dev 
...
WORKDIR ?= work
#WORKSRC ?= $(BASEDIRABS)/$(WORKDIR)/$(DISTNAME)
#WORKOBJ ?= $(BASEDIRABS)/$(WORKDIR)/$(DISTNAME)-build
...
$(sort $(DOWNLOADDIR) $(COOKIEDIR) $(LOGDIR) $(WORKSRC) $(WORKOBJ) $(WORKDIR) $(EXTRACTDIR) $(FILEDIR) $(SCRATCHDIR) $(DOWNLOADDIRGIT)) :
	@echo ------ check dir $@
	@if test -d "$@"; then : ; else \
       mkdir -p "$@"; \
     fi
...
#fetch:     pre-everything custom-pre-everything $(COOKIEDIR) $(DOWNLOADDIR) $(DOWNLOADDIRGIT) 
...
install: $(if $(USE_TESTS),test,build) $(addprefix dep-$(GARDIR)/,$(INSTALLDEPS)) prepare-install pre-install \
	    custom-pre-install $(addprefix install-,$(ALL_INSTALL_SCRIPTS)) finish-package-install pre-stow   \
	    custom-pre-stow $(addprefix sysinstall-,$(filter-out $(SYSINSTALL_IGNORE),$(SYSINSTALL_TARGETS))) \
	     post-install custom-post-install
	$(DONADA)

# returns true if install has completed successfully, false
# otherwise
install-p:
	@test -h $(dotgardir)/VERSION && test "$(shell readlink $(dotgardir)/VERSION)" = $(PACKAGENAME)
...
# FIX bzr or brz or , to come, git

#git add ./Makefile ./sha256sums ./config.mk && git commit -m 'update $(GARNAME) to $(GARVERSION)'

# Automatically find dependencies (after a configure).  ## inoperative, old
find-deps:
	$(PKG_CONFIG) --fpc-dump
...
clean:
	...
	rm --preserve-root=all -rf $(DOWNLOADDIR) $(COOKIEDIR) $(LOGDIR) $(WORKDIR) \
        $(EXTRACTDIR) $(SCRATCHDIR) $(SCRATCHDIR)-$(COOKIEDIR) *~

# cwh changed 6-2022 rm --preserve-root=all -rf $(DOWNLOADDIR) $(COOKIEDIR) $(LOGDIR) $(WORKSRC) $(WORKDIR) $(EXTRACTDIR) $(SCRATCHDIR) $(SCRATCHDIR)-$(COOKIEDIR) *~
