...
keyring: fetch
	id=`gpg --verify $(DOWNLOADDIR)/$(SIGFILES) 2>&1 | grep -o -E "[0-9A-Z]{8}"` \
	   && gpg -v    --keyserver hkp://pgp.mit.edu:80  --recv-keys $$id && gpg -v --export $$id > gpg-keyring

#  && gpg -v    --keyserver keys.gnupg.net  --recv-keys $$id && gpg -v --export $$id > gpg-keyring


# 2020 Nov 17 2020 changed back above
#&& gpg -v  --recv-keys $$id ; gpg -v --export $$id > gpg-keyring
#        && gpg -v    --keyserver keys.gnupg.net  --recv-keys $$id && gpg --export $$id > gpg-keyring
#c. change
